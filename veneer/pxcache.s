...
/*  */
        ...
        rlwimi  %r4, %r3, PAGE_SHIFT, 0xfffff000

        addi    %r5, %r5, 31            /*  bump length by block size - 1 */
        srwi    %r5, %r5, 5             /*  get number of blocks */
...
/*  */
        ...
        DISABLE_INTERRUPTS(%r12,%r11)   /*  r.11 <- disabled MSR */
                                        /*  r.12 <- previous MSR */
/*  */
...
hspribc:
        ...
        mflr    %r6                     /*  r.6 <- &hspribc */
        rlwinm  %r6, %r6, 0, 0x7fffffff /*  r.6 &= 0x7fffffff */
        addi    %r6, %r6, hspribc.real - hspribc
                                        /*  r.6 = real &hspribc.real */

        sync                            /*  ensure all previous loads and */
                                        /*  stores are complete. */

        mtsrr0  %r6                     /*  address in real space */

        rlwinm  %r11, %r11, 0, ~0x30    /*  turn off Data and Instr relocation */
        mtsrr1  %r11
        rfi                             /*  leap to next instruction */

hspribc.real:
        mtsrr0  %r0                     /*  set return address */
        mtsrr1  %r12                    /*  set old MSR value */

hspribc.loop:
/*  XXX  dcbst   0, %r4                  // flush data block to memory */
        dcbf    0, %r4                  /*  flush data block to memory */
        icbi    0, %r4                  /*  invalidate i-cache */
        addi    %r4, %r4, 32            /*  point to next block */
